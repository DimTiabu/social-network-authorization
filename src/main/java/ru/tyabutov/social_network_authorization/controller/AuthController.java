...
import ru.tyabutov.social_network_authorization.exception.JwtAuthenticationException;
import ru.tyabutov.social_network_authorization.security.AppUserDetails;
import ru.tyabutov.social_network_authorization.service.AuthService;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.*;
import ru.tyabutov.social_network_authorization.service.JwtService;
import ru.tyabutov.social_network_authorization.service.impl.RefreshTokenService;
        ...
        if (request.getCookies() != null) {
            for (Cookie cookie : request.getCookies()) {
                if ("jwt".equals(cookie.getName())) {
                    log.info("Token from cookie: {}", cookie.getValue());
                    return cookie.getValue();
                }
            }
        }

        // 3. Если ничего не найдено → ошибка
